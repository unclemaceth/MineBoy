...
/**
 * @title MiningClaimRouterV3_1
 * @dev V3.1: Adds delegate.xyz support (hot wallet can claim for cold vault)
 * @notice Changes from V3:
 *  - Added `caller` field to ClaimV3 struct (hot wallet submitting tx)
 *  - `wallet` field now represents vault/owner (receives rewards, must own cartridge)
 *  - Enables cold storage security while mining from hot wallet
 * @notice All other features unchanged:
 ...
 */
contract MiningClaimRouterV3_1 is AccessControl, EIP712, Pausable {
    using ECDSA for bytes32;
    
    // Roles
    bytes32 public constant SIGNER_ROLE = keccak256("SIGNER_ROLE");
    
    // EIP-712 type hash for ClaimV3 (V3.1: includes caller field)
    ...
    struct ClaimV3 {
        address cartridge;
        uint256 tokenId;
        address wallet;    // V3.1: vault/owner (receives rewards, must own cartridge)
        address caller;    // V3.1: hot wallet (submits tx, verified as msg.sender)
        bytes32 nonce;
        uint256 tier;
        uint256 tries;
        uint256 elapsedMs;
    ...
    function claimV3(
        ...
        require(msg.sender == claimData.caller, "Invalid caller");  // V3.1: Check hot wallet
        ...
        // Verify cartridge ownership (V3.1: vault must own, not caller)
        require(
            IERC721(claimData.cartridge).ownerOf(claimData.tokenId) == claimData.wallet,
            "Not cartridge owner"
        );
        
        // Verify EIP-712 signature (V3.1: includes caller field)
        {
            bytes32 structHash = keccak256(abi.encode(
                ...
                claimData.tokenId,
                claimData.wallet,
                claimData.caller,  // V3.1: Added caller field
                claimData.nonce,
                claimData.tier,
                claimData.tries,
                claimData.elapsedMs,
