...
CREATE TABLE IF NOT EXISTS paid_messages (
  ...
  status TEXT NOT NULL DEFAULT 'active', -- 'active', 'playing', 'expired'
  message_type TEXT NOT NULL DEFAULT 'PAID', -- 'MINEBOY', 'PAID', 'SHILL'
  nonce INTEGER,
  msg_hash TEXT,
...
COMMENT ON COLUMN paid_messages.status IS 'active (queued), playing (currently displaying), expired (finished)';
